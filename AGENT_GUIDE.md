# Video producer - Agent Guide

Start here. This is the complete operating guide and agent contract for video-producer (forked from OpenMontage — see [NOTICE](NOTICE) for attribution).
...
This is a first-class workflow in video-producer.
...
If a model misses this distinction, it will often fall back to plain search + guesswork. That is incorrect for video-producer.
...
5. **Read Layer 3 skills before calling tools.** Before using any tool with an `agent_skills` field, read the referenced skill in `.claude/skills/`. These contain provider-specific prompting guidance, parameter optimization, and quality techniques that dramatically improve output.
...
## What video-producer Is

video-producer is an instruction-driven video production system. The AI agent IS the intelligence — it reads instructions (pipeline manifests + stage director skills + meta skills) and drives the pipeline using tools.
...
| `documentary-montage` | Footage-led documentary montage | beta |
| `social-short-15s` | 15s social short (Hook+3×3s+CTA), local-GPU first via ComfyUI | beta |
...
video-producer uses two layers for capability choice:
...
video-producer has three instruction layers:

1. `tools/`
   What exists, what is available, cost, runtime, fallback, related skills.
2. `skills/`
   How video-producer wants those tools used in pipelines.
3. `.claude/skills/`
...
The `.claude/skills/` directory is large. When you're not coming in through a tool's `agent_skills` pointer, use this table to find the right file by *what you're trying to do*:
